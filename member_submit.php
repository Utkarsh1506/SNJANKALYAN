...
$row = [date('c'), $userId, $name, $gender, $sonof, $dob, $mobile, $aadhar, $email, $address, $pincode, $profession, $blood, $state, $district, $savedProfile, $savedIdDoc, $savedOther, $member_amount, $member_utr, $savedScreenshot, $status];
$fp = fopen($csvFile, 'a');
if ($fp) {
    if (ftell($fp) === 0) fputcsv($fp, ['timestamp','userid','name','gender','sonof','dob','mobile','aadhar','email','address','pincode','profession','blood','state','district','profile_pic','id_doc','other_doc','amount','utr','screenshot','status']);
    fputcsv($fp, $row);
    fclose($fp);
} else {
    // non-fatal
...
$html .= '<p>Your membership application has been received successfully. Please save these credentials to download your ID card:</p>';
...
$html .= '</ul>';
$html .= '<div class="alert alert-info mt-3"><strong>Important:</strong> Your application is currently under review. Once approved by our team, you will be able to download your ID card from the <a href="idcard.html" class="alert-link">ID Card Download</a> page.</div>';
$html .= '<p class="mt-3">Please store your User ID safely. The password is your Date of Birth as entered during application.</p>';
$html .= '</div>';
